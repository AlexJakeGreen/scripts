...
0113 2a   ld	hl, (6)
0116 f9	  ld	sp, hl
0117 11   ld	de, 1dda              ; msg1
011a 0e   ld	c, 9
011c cd	  call	1dce                ; bdos

011f	    ld	hl, 013a          ; tests: first test case

;; loop:
0122	ld	a, (hl)		; end of list ?
0123	inc	hl
0124	or	(hl)
0125	jp	z, 012f               ; done:
0128	dec	hl
...
1ae4	inc	hl
	    ld	h,(hl)
	    ld	l,a
1ae7	ld	a,(hl)		; flag mask
	    ld	(flgmsk+1),a  ; ld (1d65), a
	    inc	hl
	    push	hl
1aed	ld	de,20
	    add	hl,de		; point to incmask
	    ld	de, counter           ; 1cda
1af4  call	initmask            ; 1c49
	pop	hl
	push	hl
	ld	de,20+20
...
1b03	ld	hl,shifter
	...
	pop	hl
	push	hl
1b0a	ld	de, iut		; 1d42 copy initial instruction under test
	ld	bc,4
	ldir
	ld	de,msbt		; 0103 copy initial machine state
	ld	bc,16
	ldir
	...
	ex	de,hl
	ld	c,9
1b21	call	bdos		; show test name
1b24	call	1e71 		; initcrc initialise crc

  
; test loop
1b27	ld	a,(iut)               ; 1d42 tlp:
...
1b33	jp	nz,tlp1
	ld	a,(iut+1)
	cp	076h
1b3b tlp1:	call	nz, 1d2a 		  ; test: execute the test instruction
tlp2:	call	count		; 1c89 increment the counter
...
1c49	push	de
	ex	de,hl
	ld	bc,20+20
1c4e	call	clrmem		; 1c38 clear work area
1c51	ex	de,hl
...
1d63	ld	a,(hl)
1d64  flgmsk:	and	a,0d7h		; mask-out irrelevant bits (self-modified code!)
	ld	(hl),a
...
1d6f tcrc:	ld	a,(de)
1d70	inc	de
1d71	call	updcrc		; 1e49 accumulate crc of this test case
1d74	dec	b
1d75	jp	nz,tcrc
      if	0
	ld	e,' '
