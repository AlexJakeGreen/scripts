...
void unimplemented_instruction(uint8_t instruction) {
  printf("Not implemented: %02x\n", instruction);
  exit(EXIT_FAILURE);
}

  // d: 00: rm <- reg, 01: reg <- rm
  /* printf("\n%02x d:%d w:%d(0: byte, 1: word)\n", *code, d, w); */

  /* printf("%02x mod:%x reg:%x rm:%x\n", */
  /*        code[1], */
  /*        (code[1] & 0b11000000) >> 6, */
  /*        (code[1] & 0b00111000) >> 3, */
  /*        code[1] & 0b00000111); */
  
  /* printf("rm: %s, reg: %s\n", rm_table1[code[1] & 0b00000111], reg_w1[(code[1] & 0b00111000) >> 3]); */

int disassemble_opcode_8086(unsigned char *codebuffer, int ip) {
  unsigned char *code = &codebuffer[ip];
  int opbytes = 1;
  ...
  case 0xc6: // MOV Eb Ib
    {
      ...
      if (mod == 0b00) {
        ...
        // 11 Two register instruction; use REG table
        unimplemented_instruction(*code); // second arg is Iv
        // printf("ADD %s, %s", reg_w1[rm], reg_w1[reg]);
      } else {
        // should never be reached
        unimplemented_instruction(*code);
